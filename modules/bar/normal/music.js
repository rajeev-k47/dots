...
import cavaWidget from './cavaWidget.js';
        ...
        child: Box({
            ...
            setup: (self) => self.poll(5000, () => execAsync(['bash', '-c', command])
                .then((output) => {
                    ...
                    resourceLabel.label = `${value}${isTemp ? '°C' : '%'}`;
                    widget.tooltipText = `${name}: ${value}${isTemp ? '°C' : '%'}`;
                }).catch(print))
            ,
        })
    });
                    ...
                    Revealer({
                        ...
                        child: Box({
                            ...
                            children: [
                                ...
                                BarResource(getString('Temp'), 'thermostat', `sensors | awk '/^Package id 0:/ {print $4}' | sed 's/+//' | sed 's/°C//'`,
                    `bar-temp-circprog ${userOptions.appearance.borderless ? 'bar-temp-circprog-borderless' : ''}`, 'bar-temp-txt', 'bar-temp-icon')
                            ]
                        }),
                        setup: (self) => self.hook(Mpris, label => {
                            const mpris = Mpris.getPlayer('');
